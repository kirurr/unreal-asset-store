...
class FileController
{
    ...
    {    
        try {
            ...
            renderView('profile/assets/files/index', ['files' => $files, 'asset_id' => $id]);
        } catch (Exception $e) {
    ...
    {    
        try {
            renderView('profile/assets/files/create', ['asset_id' => $id]);
        } catch (Exception $e) {
            http_response_code(500);
            renderView('error', ['error' => $e->getMessage()]);
        } 
    ...
    {    
        ...
        header("Location: /profile/assets/$asset_id/files", true, 303);
    }
    ...
    public function showEdit(string $asset_id, string $file_id): void
    {
        try {
            ...
            renderView('profile/assets/files/edit', ['file' => $file, 'asset_id' => $asset_id]);
        } catch (Exception $e) {
            http_response_code(500);
            renderView('error', ['error' => $e->getMessage()]);
        } 
    ...
    public function update(string $asset_id, string $file_id, string $name, string $version, string $description, string $file_name, string $path, string $old_path): void
    {
        try {
            ...
            renderView('profile/assets/files/edit', ['file' => $file, 'asset_id' => $asset_id, 'errorMessage' => $e->getMessage()]);
        } catch (Exception $e) {
            http_response_code(500);
            renderView('error', ['error' => $e->getMessage()]);
        }
